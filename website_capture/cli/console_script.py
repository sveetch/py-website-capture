...
# Default logger conf
WEBSITECAPTURE_LOGGER_CONF = ('DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL', None)
...
def cli_frontend(ctx, verbose):
    ...
    levels = [item for item in WEBSITECAPTURE_LOGGER_CONF]
    levels.reverse()
    # Init the logger config
    root_logger = init_logger("py-website-capture", levels[verbose],
                              printout=printout)
